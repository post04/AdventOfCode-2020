...
var input = `muted white bags contain 4 dark orange bags, 3 bright white bags.
...
func solve(input []string) int {
	var ans = 0
	bm := map[string][]lol{}
	rbm := map[string][]string{}
	for _, rule := range input {
		parts := strings.Split(rule, " bags contain ")
		options := strings.Split(parts[1], ", ")
		values := []lol{}
		for _, option := range options {
			bbb := strings.Split(option, " ")
			c, err := strconv.Atoi(bbb[0])
			if err != nil {
				break
			}
			b := fmt.Sprintf("%s %s", bbb[1], bbb[2])
			values = append(values, lol{c, b})
			if rv, ok := rbm[b]; ok {
				rbm[b] = append(rv, parts[0])
			} else {
				rbm[b] = []string{parts[0]}
			}
		}
		bm[parts[0]] = values
	}
...
func contains(bag string, bm *map[string][]lol) int {
	total := 0
	for _, bac := range (*bm)[bag] {
		total += bac.count * (1 + contains(bac.bag, bm))
	}
	return total
}
