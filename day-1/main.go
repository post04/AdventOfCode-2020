...
func solve2(arrayOfNumbers []int) int {
	var solved int
	for _, num := range arrayOfNumbers {
		for _, num1 := range arrayOfNumbers {
			var lol = num + num1
			for _, num2 := range arrayOfNumbers {
				if lol+num2 == 2020 {
					solved = num * num1 * num2
					return solved
				}
			}
		}
...
func solve(arrayOfNumbers []int) int {
	var solved int
	for _, num := range arrayOfNumbers {
		for _, num1 := range arrayOfNumbers {
			if num+num1 == 2020 {
...
func main() {
	arrayOfNumbers := strings.Split(numbers, " ")
	arrayOfInts := convertAll(arrayOfNumbers)
	answer := solve(arrayOfInts)
	fmt.Println(answer)
	answer2 := solve2(arrayOfInts)
	fmt.Println(answer2)
}
