...
func solve(input1 string) int {
	var t = 0
	var invalid = []string{}
	for _, kekw := range strings.Split(input1, "\n") {
		if strings.ContainsAny(kekw, "abcdefghijklmnopqrstuvwxyz") == false || kekw == strings.Split(input1, "\n")[len(strings.Split(input1, "\n"))-1] {
			invalid = append(invalid, kekw)
			lmao := strings.Join(invalid, "\n")
			lll := strings.Count(lmao, ":")
			ll := strings.Count(lmao, "cid:")
			if lll < 7 {
				invalid = []string{}
			} else if lll == 7 && ll == 1 {
				invalid = []string{}
...
func solve2(input1 string) int {
	...
	for _, kekw := range strings.Split(input1, "\n") {
		if strings.ContainsAny(kekw, "abcdefghijklmnopqrstuvwxyz") == false || kekw == strings.Split(input1, "\n")[len(strings.Split(input1, "\n"))-1] {
			...
			if lll < 7 {
				...
				t++
				valid = append(valid, strings.Join(invalid, "\n"))
				invalid = []string{}
			}
		} else {
			invalid = append(invalid, kekw)
	...
	for _, v := range valid {
		var inv = false
		lmao := strings.Split(v, "\n")
		for _, lol := range lmao {

			kek := strings.Split(lol, " ")
			for _, kkk := range kek {
				tit := strings.Split(kkk, ":")
				if len(tit) > 1 && inv == false {
					if tit[0] == "byr" {
						if !(convert(tit[1]) > 1920) || !(convert(tit[1]) < 2003) {
							t--
							inv = true
						}

					} else if tit[0] == "iyr" {
						if !(convert(tit[1]) > 2009) || !(convert(tit[1]) < 2021) {
							t--
							inv = true
						}

					} else if tit[0] == "eyr" {
						if !(convert(tit[1]) > 2019) || !(convert(tit[1]) < 2031) {
							t--
							inv = true
						}

					} else if tit[0] == "hgt" {
						if strings.Contains(tit[1], "cm") {
							num := convert(strings.Split(tit[1], "cm")[0])
							if num < 150 || num > 193 {
								t--
								inv = true
							}
						} else if strings.Contains(tit[1], "in") {
							num := convert(strings.Split(tit[1], "in")[0])
							if num < 59 || num > 76 {
								t--
								inv = true
							}
						...
						str := strings.Split(tit[1], "")
						if str[0] != "#" {
							t--
							inv = true
						} else if str[0] == "#" && len(str) != 7 {

							t--
							inv = true
						}
					} else if tit[0] == "ecl" {
						var valids = []string{"amb", "blu", "brn", "gry", "grn", "hzl", "oth"}
						if !find(valids, tit[1]) {
							t--
							inv = true
						}
					} else if tit[0] == "pid" {
