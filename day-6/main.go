...
var input = `rypdeiqkbgacnxwumhtozfjvs
...
func solve(input []string) int {
	...
	for _, answers := range input {
		var answered = []string{}
		lol := strings.Split(answers, "\n")

		for _, chunk := range lol {
			for _, letter := range chunk {
				if stringInSlice(string(letter), answered) {

				} else {
					answered = append(answered, string(letter))
				}
			}
		}
		...
		for pos, chunk := range lol {
			if pos == 0 {
				...
				for _, letter := range chunk {
					...
					for _, kek := range answered {
						if string(kek) == string(letter) {
							isin = append(isin, string(letter))
						}
					}
